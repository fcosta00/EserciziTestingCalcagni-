evaluate_partial_invariance = function(fitted_model = NULL,type = c("metric","scalar","strict")){
  ...
  X = modificationIndices(object = fitted_model,free.remove = FALSE,sort. = TRUE,op = typechar)[,c(1,2,3,5,7)];rownames(X)=NULL
  print(paste0("Evaluating invariance: ",type))
  
  if(type=="metric"|type=="strict"){
    Y = parTable(fitted_model); rownames(Y)=NULL
    Y = Y[Y$op==typechar,c(2,4)]
    Y = t(apply(Y,1,as.character))
    Z = t(apply(X[,c(1,3)],1,as.character))
    n = NROW(Y)
    iid = unlist(as.vector(mapply(function(i){which(Y[i,1]==Z[,1]&Y[i,2]==Z[,2])},1:n)))
    Z = data.frame(X[unique(iid),]); Z$mi = round(Z$mi,5)
    X = Z[order(Z$mi,decreasing = TRUE),]
  }
...
summary_table = function(fitted_model = NULL,type_summary=c("all","latent->manifest","covariance","variance","intercept","threshold"),standardized=TRUE){
...
filterLambda = function(Lambda_est = NULL,thr=0.35){
  X = abs(lavInspect(object = mod1_fit,what="std")$lambda)
  out = mapply(function(q){X[X[,q]>=thr,q]},1:NCOL(X))
  names(out) = colnames(X)
  return(out)
}


hclust2lavaan = function(tree = NULL,ngroups = NULL){
  gps = cutree(tree,k = ngroups)
  X = data.frame(gps)
  res = mapply(function(j){rownames(X)[X$gps==j]},1:ngroups)
  if(ngroups==1){
    x = paste0("eta"," =~ ",paste(res,collapse = "+"))
  }else{
    x = paste(mapply(function(h){paste0(paste0("eta",h)," =~ ",paste(res[[h]],collapse = "+"))},1:ngroups),collapse = " \n ")
  }
  return(x)
}
...
lavaan_checkConvergence = function(fitted_model=NULL){
  ...
  pbs = c("Parameters estimation","Theta_delta not positive defined","Phi not positive defined","Standard errors not computed")
  ...
  c5 = sum(is.na(PT$se))>1
  iid = which(c(c1,c3,c4,c5)==1)
  if(length(iid)==0){
    message("Convergence: Ok.")
  }else{
    if(c1==TRUE | c3==TRUE | c4==TRUE | c5==TRUE){
...
plot_lavaan_model = function(fitted_model=NULL,what=c("std.all","est"),cex_label=1.2,ndigits=2,ask=FALSE,style="lisrler",...){
  require(semPlot)
  wm = match.arg(what)
  semPaths(fitted_model,nCharNodes = 3,what = "model", whatLabels = wm,
           edge.label.cex = 1.2,edge.color = "black",sizeMan = 7,sizeLat=12,style = style,nDigits = ndigits,ask = ask,...)
}
...
##############################################
## Source: https://github.com/cddesja/hemp/ ##
##############################################
